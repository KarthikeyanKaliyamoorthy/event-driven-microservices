...
public class AccountsServiceImpl  implements IAccountsService {
    ...
    public boolean updateMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        ...
        try {
            ...
            Accounts accounts = accountsRepository.findByMobileNumberAndActiveSw(currentMobileNumber, AccountsConstants.ACTIVE_SW)
            ...
            updateCardsMobileNumber(mobileNumberUpdateDto);
            result = true;
        } catch (Exception e) {
    ...
    public boolean rollbackMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        ...
        Accounts accounts = accountsRepository.findByMobileNumberAndActiveSw(newMobileNumber, AccountsConstants.ACTIVE_SW)
                .orElseThrow(() -> new ResourceNotFoundException("Account", "mobileNumber", newMobileNumber));
    ...
    private void updateCardsMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        log.info("Sending message to output channel: updateCardsMobileNumber-out-0 with payload: {}", mobileNumberUpdateDto);
        boolean result = streamBridge.send("updateCardsMobileNumber-out-0", mobileNumberUpdateDto);
        log.info("Is the message sent to output channel: updateCardsMobileNumber-out-0 ? {}", result);
    }

    private void rollBackCustomerMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        log.info("Sending message to output channel: rollBackCustomerMobileNumber-out-0 with payload: {}", mobileNumberUpdateDto);
        boolean result = streamBridge.send("rollBackCustomerMobileNumber-out-0", mobileNumberUpdateDto);
        log.info("Is the message sent to output channel: rollBackCustomerMobileNumber-out-0 ? {}", result);
    }
