...
public class CardsServiceImpl implements ICardsService {
    ...
    public boolean updateMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        ...
        try {
            ...
            Cards cards = cardsRepository.findByMobileNumberAndActiveSw(currentMobileNumber, CardsConstants.ACTIVE_SW)
    ...
    public boolean rollbackMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        ...
        Cards cards = cardsRepository.findByMobileNumberAndActiveSw(newMobileNumber, CardsConstants.ACTIVE_SW)
    ...
    private void rollbackAccountMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        log.info("Sending message to output channel: rollbackAccountMobileNumber-out-0 with payload: {}", mobileNumberUpdateDto);
        boolean result = streamBridge.send("rollbackAccountMobileNumber-out-0", mobileNumberUpdateDto);
        log.info("Is the message sent to output channel: rollbackAccountMobileNumber-out-0 ? {}", result);
    }

    private void updateLoansMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        log.info("Sending message to output channel: updateLoansMobileNumber-out-0 with payload: {}", mobileNumberUpdateDto);
        boolean result = streamBridge.send("updateLoansMobileNumber-out-0", mobileNumberUpdateDto);
        log.info("Is the message sent to output channel: updateLoansMobileNumber-out-0 ? {}", result);
    }
