...
public class CustomerServiceImpl implements ICustomerService {
    ...
    public boolean updateMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        ...
        Customer customer = customerRepository.findByMobileNumberAndActiveSw(currentMobileNumber, CustomerConstants.ACTIVE_SW)
...
//        throw new RuntimeException("Some error occurred while updating mobile number");
    ...
    private void updateAccountMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
        log.info("Sending message to output channel: updateAccountMobileNumber-out-0 with payload: {}", mobileNumberUpdateDto);
        boolean result = streamBridge.send("updateAccountMobileNumber-out-0", mobileNumberUpdateDto);
        log.info("Is the message sent to output channel: updateAccountMobileNumber-out-0 ? {}", result);
    }
